...
Это спецификация для реализации студентами. Стартовый код реализует только clock probe. До конца недели 2 группы фиксируют контракт и записывают любые изменения в ADR/PR, согласованный с обеими сторонами интерфейса.

## Координаты и время

SI units, правосторонние системы ROS: `x` вперёд, `y` влево, `z` вверх; yaw в радианах. TF: `map → odom → base_link → lidar_link / imu_link / camera_link → camera_optical_frame`. Optical frame: `z` вперёд, `x` вправо, `y` вниз. G1 владеет двумя динамическими transforms, simulator adapter — только sensor static transforms. Исключить duplicate TF publishers от Gazebo/Nav2.

Единственный источник `/clock` — симулятор через bridge. Runtime nodes используют `use_sim_time=true`. Latency вычисляется steady clock; возраст измерения — simulation time. При reset времени очистить фильтры, histories, action states, cached TF и команду; episode ID меняется. Watchdog остановки использует также steady clock, чтобы зависший `/clock` не скрывал отказ.

## Topics и actions

| Интерфейс | ROS 2 type | Owner → consumer | Rate / deadline | QoS / условие |
|---|---|---|---|---|
| `/clock` | `rosgraph_msgs/msg/Clock` | simulator → all | progressing | best effort, volatile, depth 10 |
| `/sensors/wheel_odom` | `nav_msgs/msg/Odometry` | simulator → G1 | 50 Hz | sensor data QoS |
...
| `/state/odom` | `nav_msgs/msg/Odometry` | G1 → G4/G5 | 50 Hz / 20 ms | reliable, volatile, depth 5; frame `odom`, child `base_link` |
...
| Некорректная covariance, NaN/Inf, invalid model | Reject output, classical fallback либо stop, log reason | каждая группа |
