...
Конкретное содержание S1–S4, обязательные случаи для каждой подгруппы, пять способов внесения отказов и порядок демонстрации преподавателю перечислены в [README: какие сценарии нужны для всего робота](../README.md#какие-сценарии-нужны-для-всего-робота). Это единый перечень приёмки. В частности, S4 содержит 10 эпизодов с возмущением движения и 10 с ухудшением измерений (по 5 каждого типа на каждой тестовой планировке); проверка повреждённой модели в C0 подтверждает независимость классического режима от неиспользуемого файла модели.
