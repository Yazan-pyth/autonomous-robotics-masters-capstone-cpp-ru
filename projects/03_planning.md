...
Исследовательский вопрос: помогает ли обученная стоимость проходимости выбирать маршруты с меньшими задержками, сохраняя ограничения на столкновения?
...
Пакет `capstone_planning`, обе cost-модели, map/goal tests (включая unreachable), model/data cards, replay input, metrics и отчёт. Hard collision mask одинаков для обоих методов; confidence модели не разрешает проход через занятые клетки. Обязанности по реализации, проверке и интеграции распределяются внутри группы.
