...
4. Реализация: C++ architecture, ROS (Robot Operating System — программная платформа для робототехники) contracts, dependencies, inference и fallback.
...
7. Результаты: raw/aggregate links, per-scenario metrics, CI (Confidence Interval — доверительный интервал)/effect sizes, learning curves, runtime/memory.
...
| Scenario | Method | n episodes / training seeds | Primary metric + 95% CI (Confidence Interval — доверительный интервал) | Safety | p95 (95th percentile — 95-й процентиль) latency | Fallback fraction |
|---|---|---|---|---|---|---|
| заполнить | classical | | | | | |
| заполнить | learned | | | | | |
