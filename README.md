...
## Сколько студентов участвует

**Всего в проекте 12 студентов, объединённых в одну команду из пяти подгрупп.** Каждый студент входит ровно в одну подгруппу:

| Подгруппа | Направление | Количество студентов |
|---|---|---:|
| G1 (Group 1 — подгруппа 1) | Локализация и геометрическая карта | **3** |
| G2 (Group 2 — подгруппа 2) | Восприятие и обнаружение препятствий | **2** |
| G3 (Group 3 — подгруппа 3) | Глобальное планирование маршрута | **2** |
| G4 (Group 4 — подгруппа 4) | Локальное управление и безопасность | **3** |
| G5 (Group 5 — подгруппа 5) | Управление миссиями и общая сборка | **2** |
| **Всего: 5 подгрупп** | **Один общий автономный робот** | **12** |

**3 + 2 + 2 + 3 + 2 = 12 студентов.** Каждая подгруппа реализует оба своих метода: классический и обучаемый. В интеграции участвуют все 12 студентов; отдельная шестая группа для интеграции не создаётся. G5 координирует общую сборку, а остальные подгруппы подключают и проверяют собственные компоненты. Распределение личных обязанностей приведено в [плане работы команды](docs/WORKFLOW.md).
...
6. **Подтвердить личный вклад.** Каждый из 12 студентов показывает свою реализацию, проверку или эксперимент, объясняет оба метода своей группы и одну задачу интеграции. Ссылки на изменения и взаимные проверки кода — в [индивидуальной записи вклада](templates/CONTRIBUTION.md).
